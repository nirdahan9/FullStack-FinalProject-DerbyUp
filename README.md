...
npm run test:integration  # 194 בדיקות מול Supabase — דורש .env.local
...
| `integration` | 194 | הרשאות ו-RLS מול DB אמיתי, שני לוחות התוצאות, **השכבה החיה**, **מכסות היועץ** ודאשבורד הניהול |
| `e2e` | 27 | המסע המלא בדפדפן — הרשמה → ליגה → ניחוש → **משחק חי** → **יועץ** → עיבוד → טבלה |

**552 בדיקות.** בדיקות האינטגרציה בונות עולם חד-פעמי משלהן ומוחקות אותו בסוף.
