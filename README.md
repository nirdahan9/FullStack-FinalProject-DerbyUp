...
npx supabase db push          # מריץ את 12 המיגרציות שב-supabase/migrations/
...
npm run test:integration  # 142 בדיקות מול Supabase — דורש .env.local
npm run test:e2e          # 22 בדיקות Playwright — דורש שרת מקומי
...
|---|---|---|
| `unit` | 210 | לוגיקת הניקוד, היישוב, הדירוג והוולידציות — **100% כיסוי** |
| `components` | 20 | מה שהמסך **אומר**: נקודות על אריח, מצב נעול, מי מודגש בטבלה |
| `integration` | 142 | הרשאות ו-RLS מול DB אמיתי, ושני לוחות התוצאות |
| `e2e` | 22 | המסע המלא בדפדפן — הרשמה → ליגה → ניחוש → יישוב → טבלה |

**394 בדיקות.** בדיקות האינטגרציה בונות עולם חד-פעמי משלהן ומוחקות אותו בסוף.
...
supabase/migrations/    12 מיגרציות — 12 טבלאות · 19 policies · 12 פונקציות
