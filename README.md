...
```bash
npm test                  # 270 בדיקות יחידה וקומפוננטות — לא נוגע ברשת, < שנייה
npm run test:integration  # 172 בדיקות מול Supabase — דורש .env.local
npm run test:e2e          # 25 בדיקות Playwright — דורש שרת מקומי
...
|---|---|---|
| `unit` | 236 | לוגיקת הניקוד, העיבוד, **השכבה החיה**, הדירוג והוולידציות |
| `components` | 34 | מה שהמסך **אומר**: נקודות על אריח, מצב נעול, **תוצאה חיה**, מי מודגש בטבלה |
| `integration` | 172 | הרשאות ו-RLS מול DB אמיתי, שני לוחות התוצאות, **השכבה החיה** ודאשבורד הניהול |
| `e2e` | 25 | המסע המלא בדפדפן — הרשמה → ליגה → ניחוש → **משחק חי** → עיבוד → טבלה |

**467 בדיקות.** בדיקות האינטגרציה בונות עולם חד-פעמי משלהן ומוחקות אותו בסוף.
